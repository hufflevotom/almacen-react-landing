...
import { backendUrl, whatsappNumber } from "../../../constants/enviroment";
import { Colors } from "../../../constants/styles";
import { AppProps } from "../../../app/utils/AppProps";
import { PhotoProvider, PhotoView } from "react-photo-view";

interface ProductDetailsProps extends AppProps {
  open: boolean;
...
const ProductDetails: FunctionComponent<ProductDetailsProps> = ({
  ...
  const sendMessage = () => {
    window.open(
      `https://api.whatsapp.com/send?phone=${whatsappNumber}&text=Hola%2C%20quiero%20mas%20información%20sobre%20este%20producto%3A%20%0A${product?.title}%20S%2F%20${product?.price}`,
      "_blank"
    );
  };
              ...
              >
                <Dialog.Panel className="flex w-full transform text-left text-base transition md:my-8 md:max-w-2xl md:px-4 lg:max-w-2xl">
                  <div className="relative flex w-full items-start overflow-hidden bg-white shadow-2xl">
                    ...
                    >
                      <span className="sr-only">{PageContent.close}</span>
                      <XMarkIcon className="h-6 w-6" aria-hidden="true" />
                    </button>

                    <div className="grid w-full grid-cols-1 items-start gap-y-8 gap-x-6 sm:grid-cols-12 lg:gap-x-8">
                      <div className="aspect-w-4 aspect-h-4 overflow-hidden bg-gray-100 sm:col-span-4 lg:col-span-5">
                        <PhotoProvider>
                          <PhotoView
                            src={
                              product?.imgUrl
                                ? `${backendUrl}${product.imgUrl}`
                          ...
                          >
                            <img
                              ...
                              alt={product?.title}
                              className="h-80 w-full object-cover object-center cursor-pointer"
                            />
                          </PhotoView>
                        </PhotoProvider>
                      </div>
                      <div className="py-2 lg:py-8 sm:col-span-8 lg:col-span-7">
                        <h2 className="text-2xl font-bold text-gray-900 pl-8 pr-12 lg:pl-2">
                          {product?.title}
                        </h2>
                        ...
                        >
                          <h3 id="information-heading" className="sr-only">
                            {PageContent.productInformation}
                          </h3>
