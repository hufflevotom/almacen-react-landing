...
const Product: FunctionComponent<ProductProps> = ({ product }) => {
  ...
  useEffect(() => {
    if (open === false) dispatch(selectProduct(null));
  }, [open]);

  return (
    <>
      <div
        className="w-full h-full flex flex-col justify-center items-center hover:opacity-95 hover:shadow-lg hover:cursor-pointer"
        onClick={() => {
      ...
      >
        <img
          ...
          className="h-60 w-full object-cover object-center align-middle border-none"
          style={{ backgroundColor: Colors[3] }}
        />
        <div className="w-full flex flex-col items-center mb-3 mt-3 px-4">
      ...
      {open ? <ProductDetails open={open} setOpen={setOpen} /> : null}
    </>
  );
};
